...
public class LogColorVariator extends ColorVariator {
	...
	float[] colors = baseColor.getColorComponents(null);
		...
		while(distance > baseDistance)
		{
			...
			colors[0] -= Math.log(distance - baseDistance)/Math.log(baseDistance);
			colors[1] -= Math.log(distance - baseDistance)/Math.log(baseDistance);
			colors[2] -= Math.log(distance - baseDistance)/Math.log(baseDistance);
			distance--;
		}
