...
public class Camera {
	...
	public Camera(Point3f pos, Vector3f dir, Point2i dim)
	{
		position = pos;
		direction = dir;
		dir.normalize();
	...
	{
		BufferedImage im = new BufferedImage(dimensions.x, dimensions.y, BufferedImage.TYPE_INT_RGB);
		for(int i = 0; i < dimensions.x; i++)
		{
			for(int j = 0; j < dimensions.y; j++)
			{
				float red = 0, green = 0, blue= 0, coef = 1.0f;
				Ray ray = new Ray(direction,new Point3f( i,dimensions.y - j, -200)); 
				int level = 0;
				...
				do{
					...
					if(ray.hit())
					{
						...
						for (Light l : Scene.lights) {
							...
							Vector3f intersectionToLight = new Vector3f(l.position.x - ray.intersectionPoint.x,
							...
							//Creo un rayo de luz desde el punto de interseccion hasta la luz
							...
							{
									if(!lightRay.isPointInSegment(lightRay.position, l.position, lightRay.intersectionPoint))
										lightHit = false;
									else
										lightHit = false;
							}
							...
							{		
									//Lambert
									//Calculo el coseno de la luz que incide sobre la superficie
									//Si esta entra perpendicular, lo ilumina mas
									//Saco el coseno mediante producto y lo multiplico por el coef
									//que se va modificando con la reflectividad del material.
									float lambert = (lightRay.direction.dot(n)) * coef;

									float[] objectColor = ray.getObject().getColor().getColorComponents(null);
